...
{ // adicionar no vetorord
    if (vetor->P < vetor->N)
    { // se o valor p for menor q N ele faz o if
        if (vetor->P == 0)
            vetor->elems[0] = newelem; // se P for 0, ele adiciona na primeira casa
        ...
        { // se newelem for menor
            ...
            { // vai repetir até entcontrar
                if (vetor->comparador(newelem, vetor->elems[i]) == 1)
                {                                          // se o elemnto for menor que newelem
